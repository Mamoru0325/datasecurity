    ...
    <div class="container" style="height: 50dvb;">
      <div class="dropdown d-flex flex-row-reverse">
        <select name="score" id="score" v-model="score" class="form-select form-control" style="width: 20%;" @click="getData">
          <option value="eazy" required>ระดับง่าย</option>
      ...
      <table cellspacing="0">
        ...
        <tbody v-for="(item, index) in paginatedData" :key="index">
          <tr>
            ...
            <td style="text-align: center"><button class="button-33" :value=item.userId @click="edit(item.userId)">เพิ่ม
                Admin</button></td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  computed: {
    ...
    async getData() {
      ...
      axios.get("http://localhost:8080/api/type/", { headers: { "Content-Type": "application/json", "Authorization": 'Bearer ' + token } })
        .then((response) => {
      ...
      const token = this.$store.state.auth.user.accessToken;
      axios.put("http://localhost:8080/api/type/" + id, feedback, { headers: { "Content-Type": "application/json", "Authorization": 'Bearer ' + token } })
        .then(() => {
